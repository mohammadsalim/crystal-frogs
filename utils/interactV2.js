...
const genesisWhitelist = require('../scripts/genesisWhitelist.js')

const web3 = createAlchemyWeb3(process.env.NEXT_PUBLIC_ALCHEMY_RPC_URL)
import { config } from '../dapp.config'

const contract = require('../artifacts/contracts/CrystalFrogsV2.sol/CrystalFrogsV2.json')
...
//genesis sale check
export const isGenesisSaleState = async () => {
  const genesisSale = await nftContract.methods.genesissaleM().call()
  return genesisSale
}
...
//get genesis price
export const getGenesisPrice = async () => {
  const price = await nftContract.methods.price().call()
  return price
}
...
export const genesissaleMint = async (mintAmount) => {
  ...
  const proof = genesisMerkleTree.getHexProof(leaf)

  // Verify Merkle Proof
  const isValid = genesisMerkleTree.verify(proof, leaf, genesisRoot)

  if (!isValid) {
    return {
      success: false,
      status: 'Invalid Merkle Proof - You do not own a Founding Frog'
    }
  }
  ...
  const tx = {
    ...
    data: nftContract.methods
      .genesisSaleMint(window.ethereum.selectedAddress, mintAmount, proof)
      .encodeABI(),
